...
class DatabaseConnector:
    ...
    def update_visitor_information(self, uuid: str):
        try:
            self.cursor.execute(f"update streamlit_website_statistics set visitor_count = {self.get_visitor_count() + 1} where id = 1")
            self.cursor.execute(f"insert into streamlit_website_visitors (uuid) values ('{uuid}')")
            self.connector.commit()
    ...
    def update_clicker_information(self, uuid: str):
        try:
            self.cursor.execute(f"update streamlit_website_statistics set clicker_count = {self.get_clicker_count() + 1} where id = 1")
            self.cursor.execute(f"insert into streamlit_website_clickers (uuid) values ('{uuid}')")
            self.connector.commit()
